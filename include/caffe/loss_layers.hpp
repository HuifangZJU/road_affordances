...
/**
 * @brief Computes the multiple binary accuracy multi-label
 *        classification task.
 */
template <typename Dtype>
class MultiLabelAccuracyLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides MultiLabelAccuracyParameter accuracy_param,
   *     with AccuracyLayer options:
   *     set probability threshold for correct predictions.
   */
...
// Forward declare MultilogisticLayer for use in MultilogisticWithLossLayer.
template <typename Dtype> class MultilogisticLayer;

template <typename Dtype>
class MultilogisticWithLossLayer : public LossLayer<Dtype> {
 public:
   /**
    * @param param provides LossParameter loss_param, with options:
    *  - ignore_label (optional)
    ...
    *    present; otherwise the loss is simply summed over spatial locations.
    */
  explicit MultilogisticWithLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "MultilogisticWithLoss"; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }

 protected:
  /// @copydoc MultilogisticWithLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  //virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    //  const vector<Blob<Dtype>*>& top);
  ...
  //virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
    //  const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  /// The internal MultilogisticLayer used to map predictions to a probability.
  shared_ptr<Layer<Dtype> > multilogistic_layer_;
  /// prob stores the output probability predictions from the MultilogisticLayer.
  Blob<Dtype> prob_;
  /// bottom vector holder used in call to the underlying MultilogisticLayer::Forward
  vector<Blob<Dtype>*> multilogistic_bottom_vec_;
  /// top vector holder used in call to the underlying MultilogisticLayer::Forward
  vector<Blob<Dtype>*> multilogistic_top_vec_;
  /// Whether to ignore instances with a certain label.
  bool has_ignore_label_;
  /// The label indicating that an instance should be ignored.
  int ignore_label_;
  /// Whether has unknow instances.
  bool has_unknow_label_;
  /// The label indicating unknow case.
  int unknow_label_;
  /// Whether to normalize the loss by the total number of values present
  /// (otherwise just by the batch size).
  bool normalize_;
  int multilogistic_axis_, outer_num_, inner_num_;
};
/**
