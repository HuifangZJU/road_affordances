...
/** huifang 
 * @brief Computes the multi logistic function.
 *
 * TODO(dox): thorough documentation for Forward, Backward, and proto params.
 */
template <typename Dtype>
class MultilogisticLayer : public Layer<Dtype> {
 public:
  explicit MultilogisticLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "Multilogistic"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
...
#ifdef USE_CUDNN
template <typename Dtype>
class CuDNNMultilogisticLayer : public MultilogisticLayer<Dtype> {
 public:
  explicit CuDNNMultilogisticLayer(const LayerParameter& param)
      : MultilogisticLayer<Dtype>(param), handles_setup_(false) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual ~CuDNNMultilogisticLayer();
