...
class DriveDataLayer : public DataLayer<Dtype> {
 ...
 public:
    ...
    virtual inline int MaxTopBlobs() const { return 5; }    //Seokju Lee, 3 -> 4 -> 5
...
template <typename Dtype>
class AffordanceDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit AffordanceDataLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~AffordanceDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "AffordanceData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
